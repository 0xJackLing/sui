        ...
        self.validators
            ...
            .map(|validator| {
                (
                    ...
                    (
                        validator.metadata.pubkey_bytes.clone(),
...
impl SuiSystemState {
    /// Always return the initial (genesis) version of the system state object.
    /// Genesis always uses this version regardless of the current version.
    pub fn into_genesis_version(self) -> SuiSystemStateInnerV1 {
        match self {
            SuiSystemState::V1(inner) => inner,
        }
    }

    /// Always treat this as the latest version and return it.
    /// Update the return type of this function whenever we upgrade the system state type.
    pub fn into_latest_version(self) -> SuiSystemStateInnerV1 {
        match self {
            SuiSystemState::V1(inner) => inner,
        }
    }
    ...
    S: ObjectStore,
{
    ...
    match wrapper.version {
        ...
        _ => panic!("Unsupported Sui System State version: {}", wrapper.version),
    }
}
